...
stored = st.session_state.get("parent_data") or {}  # ✅ None 방지 + 기존 저장된 부모 정보 불러오기

# ==== 🔧 안전한 기본값 인덱스 계산 함수 (추가) ====
...
with st.form("parent_form"):
    ...
    with col1:
        ...
        부모_희망직무 = st.selectbox(
            "희망 직무 *",
            options,
            index=default_index(options, stored.get("부모_희망직무", options[0]), 0),
    ...
    with col2:
        ...
        부모_자녀진로변화허용도 = st.selectbox(
            ...
            help="자녀가 진로 계획을 변경하려 할 때 허용해줄 수 있는 수준을 선택해주세요."
        )
    ...
    with col1:
        ...
        부모_현재직무 = st.selectbox(
            "부모 현재 직무 *",
            options,
            index=default_index(options, stored.get("부모_현재직무", options[0]), 0),
    ...
    with col1:
        ...
        부모_자녀강점유형 = st.selectbox(
            "자녀 강점 유형 *",
            options,
            index=default_index(options, stored.get("부모_자녀강점유형", options[0]), 0),
